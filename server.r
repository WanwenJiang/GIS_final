...
data.with.pop <- mutate(data, malaria.deaths.per.1000 = (X2017.malaria.deaths * (1000/X2017.pop)), 
               malaria.cases.per.1000 = (X2017.malaria.cases * (1000/X2017.pop)))
...
summary.region <- group_by(data.with.zeros, Region) %>% 
  summarise(mean.deaths = mean(X2017.malaria.deaths), mean.cases = mean(X2017.malaria.cases))
